...
export async function GET(
  ...
  try {
    ...
    // Blog private (isPublic: false) chỉ chủ blog mới xem được — trước đây
    // route này trả full nội dung cho bất kỳ ai biết ID, không check gì cả.
    if (!blog.isPublic) {
      const user = getCurrentUser(request as any);
      if (!user || user.userId !== blog.userId) {
        return NextResponse.json({ error: "Không tìm thấy blog" }, { status: 404 });
      }
    }
