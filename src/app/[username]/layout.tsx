...
import { Home, Code2, Brain, FolderGit2, Newspaper, MessageCircle, Bell, Settings, Zap, LogOut, Plus, X, FilePlus, PenSquare, Bot, BookOpen, Activity, Search } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#0a0e1a] text-gray-100">
      ...
      <div className="md:ml-64">
        <header className="sticky top-0 z-30 flex items-center justify-between px-4 md:px-6 py-4 backdrop-blur-xl bg-[#0a0e1a]/50 border-b border-white/5">
          ...
          <button
            onClick={() => window.dispatchEvent(new CustomEvent("nimbora:command-palette:open"))}
            className="hidden md:flex items-center gap-2 text-sm text-gray-400 hover:text-white transition bg-white/5 hover:bg-white/10 px-3 py-1.5 rounded-full border border-white/5"
          >
            <Search size={14} />
            <span>Tìm kiếm...</span>
            <kbd className="text-xs text-gray-500 border border-white/10 rounded px-1.5 ml-2">⌘K</kbd>
          </button>
