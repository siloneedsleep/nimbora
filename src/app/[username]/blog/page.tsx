...
import { useUndoDelete } from "@/components/ui/UndoToastProvider";
...
export default function BlogPage() {
  ...
  const submitBlog = async () => {
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || "Lỗi tạo blog");
  ...
  const handleDeleteBlog = (blog: Blog) => {
    // Ẩn ngay khỏi list (optimistic), không cần confirm() — có 5s để Undo
    ...
    scheduleDelete({
      id: blog.id,
      label: blog.title,
      onUndo: () => setBlogs((prev) => [blog, ...prev]),
      onCommit: async () => {
