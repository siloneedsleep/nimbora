...
export default function EditorPage() {
  ...
  const handleExport = async () => {
    ...
    const projectName = window.prompt(
      "Export vào project nào? (nhập tên — nếu chưa có sẽ tự tạo mới)",
      selectedFile.name.replace(/\.[^.]+$/, "")
    );
    if (!projectName || !projectName.trim()) return; // user bấm Cancel

    // Tìm project trùng tên của chính user này, không thì tạo mới
    const projectsRes = await fetch("/api/projects");
    const projectsData = await projectsRes.json();
    let project = (projectsData.projects || []).find(
      (p: { name: string }) => p.name === projectName.trim()
    );
    ...
    if (!project) {
      ...
      if (!createRes.ok) throw new Error(createData.error || "Lỗi tạo project");
    ...
    if (!fileRes.ok) throw new Error(fileData.error || "Lỗi export file");
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <ProgressButton onClick={handleExport} disabled={!selectedFile} icon={<Download size={16} />}>
            Export
          </ProgressButton>
          <button
